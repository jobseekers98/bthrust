...
@Component({
  ...
  selector: 'generic-drive-icons-bottom-quickaccess',
  template: `
	<h2 class="quick-bottom-h2" [ngSwitch]="Name.split('.')[1].toString().toLowerCase()" title="{{Name}}">
		...
		<ng-content *ngIf="Name.length < 15; then desc1; else desc2"></ng-content>
  ...
  .quick-bottom-h2 i{color:green;padding-right:5px;padding-bottom:5px;font-size:13px;}
   small{color: #6f6666;font-size: 11px;padding:3px;font-weight: 600;font-style: initial;text-align: center;display: table-cell;}
  `]
})
export class DriveIconsBottomQuickAccessComponent {
  @Input() Name: string;
